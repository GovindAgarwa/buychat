 ...
 */
public class GreryFashBeautyElecFragment extends Fragment implements CustomClickListner,Callback<JsonObject>,SwipeRefreshLayout.OnRefreshListener {
    ...
    RestBukasLaundGroceryFashionBeautyAdapter adapter;
    ...
    private void ApiCall(){
        ...
        if(getArguments().getString(Keys.city_market_id).equals(Constants.DEFAULT_STRING)) {
            Call<JsonObject> call1 = service1.getShopList(Parse.getCityIdAndCategoryIdWithLimitOffset(
                   getArguments().getString(Keys.category_id),
                    Constants.INT_TEN, Constants.DEFAULT_INT));
            call1.enqueue(this);
        }else{
            Call<JsonObject> call1 = service1.getShopList(Parse.getCityIdAndCategoryIdWithLimitOffset(
                    getArguments().getString(Keys.city_market_id),
                    getArguments().getString(Keys.category_id),
                    Constants.INT_TEN, Constants.DEFAULT_INT));
            call1.enqueue(this);
        }
    ...
    public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
        ...
        adapter = new RestBukasLaundGroceryFashionBeautyAdapter(getActivity(),arraylistCats,GreryFashBeautyElecFragment.this);
    ...
    private void ScrollApiCall(int current_page) {
        ...
        if(getArguments().getString(Keys.city_market_id).equals(Constants.DEFAULT_STRING)) {
            call = service.getShopList(Parse.getCityIdAndCategoryIdWithLimitOffset(
                    getArguments().getString(Keys.category_id),
    ...
    public void itemClickListner(int position, View view) {
        ...
        DataSingleton.getInstance().getData().setOrder_type(arrayList);
    ...
    public void filter(String query){
        ...
        if(arraylistCats.size() != 0) {
            ...
            adapter = new RestBukasLaundGroceryFashionBeautyAdapter(getActivity(), filteredList, GreryFashBeautyElecFragment.this);
            recyclerView.setAdapter(adapter);
            adapter.notifyDataSetChanged();
        }
    }
