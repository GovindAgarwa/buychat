 ...
 */
public class MovieRoomFragment extends Fragment implements Callback<JsonObject>,RecyclerItemClickListener.OnItemClickListener,SwipeRefreshLayout.OnRefreshListener{


    public static MovieRoomFragment newInstance(int positions,ArrayList<CategoriesPojos> categoriesPojoses,int pos) {
  //      position =pos;
 //       arraylistCats = categoriesPojoses;
 //       mainposition = positions;
        Bundle args = new Bundle();
        MovieRoomFragment fragment = new MovieRoomFragment();
...
//    private static int position;
    ...
    public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
        ...
        adapter = new MovieRoomAdapter(getActivity(),arrayList);
    ...
    private void ScrollApiCall(int current_page) {
        ...
        Call<JsonObject> call = service.getBrands(Parse.getMerchantIdAndSubCategoryIdWithLimitOffset(DataSingleton.getInstance().getData().getId()
    ...
    public void onItemClick(View view, int position) {
        ...
        if (BuyChat.dbHelper.getCartProductByProductId(DataSingleton.getInstance().getData().getId(), arrayList.get(position).getId()).isEmpty()) {
            BuyChat.dbHelper.InsertFeeds(arrayList.get(position));
        } else {
            BuyChat.dbHelper.UpdateFeeds(arrayList.get(position));
        }

        getActivity().setResult(Constants.INT_EIGHT);
        getActivity().finish();
