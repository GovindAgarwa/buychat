 ...
 */
public class EditAddressFragment extends Fragment implements RadioGroup.OnCheckedChangeListener, Callback<JsonObject> {
    ...
    @BindView(R.id.add_address)Button button;
...
//        initializeRecyclerView();
        button.setText("EDIT ADDRESS");
        if(DataSingleton.getInstance().getAddressData().getTag_address().equalsIgnoreCase("HOME")){
            addressOther.setVisibility(View.GONE);
            radioGroup.check(R.id.home_radio);
        }else if(DataSingleton.getInstance().getAddressData().getTag_address().equalsIgnoreCase("WORK")){
            radioGroup.check(R.id.work_radio);
            addressOther.setVisibility(View.GONE);
        }else {
            radioGroup.check(R.id.other_radio);
            addressOther.setVisibility(View.VISIBLE);
            addressOther.setText(DataSingleton.getInstance().getAddressData().getTag_address());
        }

        addressBuildingName.setText(DataSingleton.getInstance().getAddressData().getFlat_no_floor_name());
        addressLandmark.setText(DataSingleton.getInstance().getAddressData().getLandmark());
        ...
        String city_name = DataSingleton.getInstance().getAddressData().getLocality();
        citySpinner.setSelection(Parse.parseCityString(DataSingleton.getInstance().getCityData()).indexOf(city_name));
...
//        }
    ...
    public void addAddress(){
        ...
        if(isOther) {
            ...
            BuyChat.dbHelper.UpdateAddress(DataSingleton.getInstance().getAddressData().getId(),address);
        }else{
            Address address = new Address();
            address.setAccess_token(BuyChat.readFromPreferences(getActivity(),Keys.access_token,Constants.DEFAULT_STRING));
            address.setLocality( citySpinner.getSelectedItem().toString());
...
//        if(isOther) {
//           call = service.updateAddress(Parse.updateAddressData(citySpinner.getSelectedItem().toString(),addressBuildingName.getText().toString(),addressLandmark.getText().toString(),addressOther.getText().toString()));
//        }else {
//            call = service.updateAddress(Parse.updateAddressData(citySpinner.getSelectedItem().toString(),addressBuildingName.getText().toString(),addressLandmark.getText().toString(),locationName));
//        }
//        call.enqueue(this);
