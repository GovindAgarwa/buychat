...
public class CountryCodePicker extends RelativeLayout {
    ...
    void refreshPreferredCountries() {
        ...
        if (preferredCountries != null) {
            Log.d("preference list", preferredCountries.size() + " countries.xml");
    ...
    void refreshCustomMasterList() {
        ...
        if (customMasterCountriesList != null) {
            Log.d("custom master list:", customMasterCountriesList.size() + " countries.xml");
    ...
    /**
     * To provide definite set of countries.xml when selection dialog is opened.
     * Only custom master countries.xml, if defined, will be there is selection dialog to select from.
     * To set any country in preference, it must be included in custom master countries.xml, if defined
     * When not defined or null or blank is set, it will use library's default master list
     * Custom master list will only limit the visibility of irrelevant country from selection dialog. But all other functions like setCountryForCodeName() or setFullNumber() will consider all the countries.xml.
     *
     * @param customMasterCountries is country name codes separated by comma. e.g. "us,in,nz"
     *                              if null or "" , will remove custom countries.xml and library default will be used.
     */
    ...
    /**
     * This will match name code of all countries.xml of list against the country's name code.
     *
     * @param country
     * @param countryList list of countries.xml against which country will be checked.
    ...
    /**
     * This will set preferred countries.xml using their name code. Prior preferred countries.xml will be replaced by these countries.xml.
     * Preferred countries.xml will be at top of country selection box.
     * If more than one countries.xml have same country code, country in preferred list will have higher priory than others. e.g. Canada and US have +1 as their country code. If "us" is set as preferred country then US will be selected whenever setCountryForPhoneCode(1); or setFullNumber("+1xxxxxxxxx"); is called.
